...
export async function POST(req: NextRequest) {
  ...
  const { prompt, count, provider, model, size } = parse.data
  ...
  if (provider === 'pollinations' || !process.env.OPENAI_API_KEY) {
    ...
    const out = Array.from({ length: count }).map((_, i) => {
      ...
      const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=${width}&height=${height}&seed=${seed}&model=${m}`
      return { url, seed }
    })
  ...
  try {
    ...
    try {
      ...
      const out = Array.from({ length: count }).map((_, i) => {
        const seed = String(Date.now() + i)
        const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=${width}&height=${height}&seed=${seed}&model=flux`
        return { url, seed }
      })
      return NextResponse.json({ images: out })
    } catch (e) {
