    ...
    if (!raw) {
      ...
      if (q) {
        fetch('/api/generate', {
          ...
          body: JSON.stringify({ prompt: q, count: n ? parseInt(n) : 4, size: (s as any) || '1024x1024', provider: 'pollinations', model: 'flux', style: st === 'comic' ? 'comic' : st === 'ghibli' ? 'ghibli' : 'realistic' })
        })
          .then(async (r) => {
